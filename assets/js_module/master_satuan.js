...
$(document).ready(function() {
	...
	//datatables
	table = $('#tabel_satuan').DataTable({
		responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: true,
		ajax: {
			url  : base_url + "master_satuan/list_satuan",
			type : "POST" 
		},
...
function add_menu()
{
	...
	$('#modal_title').text('Tambah Master Satuan'); 
}
...
function edit_satuan(id)
{
    ...
    $.ajax({
        url : base_url + 'master_satuan/edit_satuan',
        type: "POST",
        dataType: "JSON",
        data : {id:id},
        success: function(data)
        {
            ...
            // });
            $('[name="id_satuan"]').val(data.old_data.id);
            $('[name="nama"]').val(data.old_data.nama);
            $('[name="kode"]').val(data.old_data.kode);
    
            $('#modal_satuan_form').modal('show');
	        $('#modal_title').text('Edit Master Satuan'); 
...
function save()
{
    ...
    if(save_method == 'add') {
        url = base_url + 'master_satuan/add_data_satuan';
        txtAksi = 'Tambah master Satuan';
        var alert = "Menambah";
    }else{
        url = base_url + 'master_satuan/update_data_satuan';
        txtAksi = 'Edit Master Satuan';
        var alert = "Mengubah";
    }
...
function delete_satuan(id){
    swalConfirmDelete.fire({
        ...
        if (result.value) {
            $.ajax({
                url : base_url + 'master_satuan/delete_satuan',
                type: "POST",
                dataType: "JSON",
                data : {id:id},
                success: function(data)
                {
                    swalConfirm.fire('Berhasil Hapus Data Satuan!', data.pesan, 'success');
                    table.ajax.reload();
                },
                error: function (jqXHR, textStatus, errorThrown)
                {
